...
#
# Auto-detects which ecosystems are present (Python services, Rust crate,
# Node project) and updates everything found. Layout is detected at startup
# and printed for transparency; override any detected value by exporting
# PYTHON_SERVICES / RUST_CRATE_DIR / NODE_DIR before invoking the script.
#
# What this script updates:
# - Python version across all project files (with --python <version>)
# - Python package dependencies via uv (all version types)
# - Rust crate dependencies (lock file only; --major also bumps Cargo.toml)
# - Node.js dependencies (npm)
# - UV package manager version in Dockerfiles and GitHub workflows
# - Pre-commit hooks to latest versions (frozen, SHA-pinned)
...
# Show usage — prints the header comment block (everything from the top of
# the file up to the first non-comment, non-blank line).
show_help() {
  awk '
    NR == 1 && /^#!/ { next }            # skip shebang
    /^[[:space:]]*$/ { print ""; next }  # echo blank lines so layout is preserved
    /^#/ { sub(/^# ?/, ""); print; next }
    { exit }
  ' "$0"
  exit 0
...
# Discover which ecosystems are present and which subdirectories to operate on.
# Override any of these by exporting the variable before running the script:
#   PYTHON_SERVICES=("a" "b" "c")  ./scripts/update-project.sh
#   RUST_CRATE_DIR="crates/foo"    ./scripts/update-project.sh
#   NODE_DIR="frontend"            ./scripts/update-project.sh

# PYTHON_SERVICES: directories (relative to repo root) containing pyproject.toml,
# excluding the root and common noise dirs. Empty for single-package layouts.
if [[ -z "${PYTHON_SERVICES+set}" ]]; then
  PYTHON_SERVICES=()
  while IFS= read -r -d '' pyproject; do
    dir=$(dirname "$pyproject")
    PYTHON_SERVICES+=("$dir")
  done < <(find . -maxdepth 3 -name pyproject.toml \
...
# RUST_CRATE_DIR: where Cargo.toml lives. Workspace root if it has `[workspace]`,
# otherwise the first per-crate Cargo.toml found. Empty when no Rust present.
if [[ -z "${RUST_CRATE_DIR+set}" ]]; then
  RUST_CRATE_DIR=""
  if [[ -f Cargo.toml ]]; then
    RUST_CRATE_DIR="."
  else
    while IFS= read -r -d '' cargo; do
      RUST_CRATE_DIR=$(dirname "$cargo")
      break
    done < <(find . -maxdepth 2 -name Cargo.toml \
      -not -path './target/*' \
      -print0 2>/dev/null)
  fi
fi

# NODE_DIR: where package.json lives. Empty when no Node present.
if [[ -z "${NODE_DIR+set}" ]]; then
  NODE_DIR=""
  if [[ -f package.json ]]; then
    NODE_DIR="."
  else
    while IFS= read -r -d '' pkg; do
      NODE_DIR=$(dirname "$pkg" | sed 's|^\./||')
      break
...
print_info "Detected layout:"
print_info "  Python services: ${#PYTHON_SERVICES[@]} (${PYTHON_SERVICES[*]:-<none>})"
print_info "  Rust crate dir:  ${RUST_CRATE_DIR:-<none>}"
print_info "  Node dir:        ${NODE_DIR:-<none>}"

# Helper: yields full path to per-Node manifests, or empty if no Node.
node_manifest() { [[ -n "$NODE_DIR" ]] && echo "$NODE_DIR/package.json"; }
node_lockfile() { [[ -n "$NODE_DIR" ]] && echo "$NODE_DIR/package-lock.json"; }

# Helper: yields full path to Cargo.toml, or empty if no Rust.
rust_manifest() { [[ -n "$RUST_CRATE_DIR" ]] && echo "$RUST_CRATE_DIR/Cargo.toml"; }
# ─── End layout detection ───────────────────────────────────────────────────
...
verify_dependency_updates() {
  ...
  # Rust dependencies
  if [[ -n "$RUST_CRATE_DIR" ]] && [[ -f "$(rust_manifest)" ]]; then
  ...
  if [[ -n "$NODE_DIR" ]] && [[ -f "$(node_manifest)" ]]; then
    print_success "✓ Node.js dependencies ($(node_manifest) + $(node_lockfile))"
  fi
  ...
  if [[ "$DRY_RUN" == false ]]; then
    ...
    if [[ -n "$RUST_CRATE_DIR" ]] && [[ -f "$(rust_manifest)" ]]; then
      print_info "Run 'cd $RUST_CRATE_DIR && cargo update --dry-run' to verify Rust crates"
    fi
  fi
}
...
update_node_packages() {
  if [[ -z "$NODE_DIR" ]] || [[ ! -f "$(node_manifest)" ]]; then
  ...
  if [[ "$BACKUP" == true ]] && [[ "$DRY_RUN" == false ]]; then
    backup_file "$(node_manifest)"
    backup_file "$(node_lockfile)"
  fi

  if [[ "$DRY_RUN" == false ]]; then
    print_info "Updating npm packages in $NODE_DIR/ (via just for single-source-of-truth)..."

    if just update-npm; then
      print_success "npm packages updated successfully"
      FILE_CHANGES+=("$(node_manifest): Updated npm dependencies")
      FILE_CHANGES+=("$(node_lockfile): Updated npm lockfile")
      CHANGES_MADE=true
    else
...
update_rust_crates() {
  if [[ -z "$RUST_CRATE_DIR" ]] || [[ ! -f "$(rust_manifest)" ]]; then
    print_info "No Rust crate found, skipping Rust updates"
    return
  fi
  ...
  if [[ "$BACKUP" == true ]] && [[ "$DRY_RUN" == false ]]; then
    backup_file "$(rust_manifest)"
    [[ -f Cargo.lock ]] && backup_file "Cargo.lock"
    [[ -f "$RUST_CRATE_DIR/Cargo.lock" ]] && backup_file "$RUST_CRATE_DIR/Cargo.lock"
  fi
    ...
    if [[ "$MAJOR_UPGRADES" == true ]]; then
      ...
      if [[ "$has_cargo_upgrade" == true ]]; then
        ...
        if cargo upgrade --incompatible allow 2>&1; then
          ...
          FILE_CHANGES+=("$(rust_manifest): Updated dependency version requirements (major)")
          CHANGES_MADE=true
...
update_python_packages() {
  ...
  if [[ "$BACKUP" == true ]] && [[ "$DRY_RUN" == false ]]; then
    ...
    # Backup all per-service pyproject.toml files (auto-detected)
...
run_tests() {
  ...
  # Run JavaScript tests if a Node project exists
  if [[ -n "$NODE_DIR" ]] && [[ -f "$(node_manifest)" ]]; then
  ...
  # Run Rust tests if a Rust crate exists. Recipe name varies by repo —
  # try `just test-rust` first (recommended convention), fall back to the
  # legacy `just test-extractor` for discogsography-shaped repos.
  if [[ -n "$RUST_CRATE_DIR" ]] && [[ -f "$(rust_manifest)" ]]; then
    print_info "Running Rust tests..."
    if just --show test-rust >/dev/null 2>&1; then
      RUST_TEST_RECIPE="test-rust"
    elif just --show test-extractor >/dev/null 2>&1; then
      RUST_TEST_RECIPE="test-extractor"
    else
      RUST_TEST_RECIPE=""
    fi

    if [[ -n "$RUST_TEST_RECIPE" ]] && just "$RUST_TEST_RECIPE"; then
      print_success "Rust tests passed"
    else
      print_warning "Rust tests failed or no recipe found (looked for test-rust, test-extractor)"
    fi
  fi
}
...
generate_summary() {
  ...
  if [[ -n "$UV_VERSION_CHANGE" ]] || [[ $(array_length FILE_CHANGES) -gt 0 ]]; then
    echo "   git add Dockerfile */Dockerfile */*/Dockerfile"
    [[ -f docs/dockerfile-standards.md ]] && echo "   git add docs/dockerfile-standards.md"
  fi
...
show_file_report() {
  ...
  # Python files (auto-detected)
  echo "🐍 Python Configuration:"
  [[ -f pyproject.toml ]] && echo "  ✓ pyproject.toml (root)"
  for service in "${PYTHON_SERVICES[@]}"; do
    [[ -f "$service/pyproject.toml" ]] && echo "  ✓ $service/pyproject.toml"
  done
  [[ -f uv.lock ]] && echo "  ✓ uv.lock (root)"
  [[ -f pyrightconfig.json ]] && echo "  ✓ pyrightconfig.json"
  echo ""

  # Docker files (auto-detected)
  local dockerfiles=()
  while IFS= read -r -d '' f; do
    dockerfiles+=("$(echo "$f" | sed 's|^\./||')")
  done < <(find . -maxdepth 3 -name Dockerfile \
    -not -path './.venv/*' -not -path './node_modules/*' \
    -not -path './target/*' -not -path './.worktrees/*' \
    -not -path './backups/*' -print0 2>/dev/null)
  if [[ ${#dockerfiles[@]} -gt 0 ]]; then
    echo "🐳 Docker Configuration:"
    for f in "${dockerfiles[@]}"; do echo "  ✓ $f"; done
    for compose in docker-compose.yml docker-compose.prod.yml docker-compose.override.yml; do
      [[ -f "$compose" ]] && echo "  ✓ $compose"
    done
    echo ""
  fi
  ...
  if [[ -n "$RUST_CRATE_DIR" ]] && [[ -f "$(rust_manifest)" ]]; then
    ...
    if [[ "$MAJOR_UPGRADES" == true ]]; then
      echo "  ✓ $(rust_manifest) (version requirements updated for major)"
    else
      echo "  ✓ $(rust_manifest) (constraints unchanged)"
    fi
    [[ -f Cargo.lock ]] && echo "  ✓ Cargo.lock (updated to latest compatible)"
    [[ -f "$RUST_CRATE_DIR/Cargo.lock" ]] && echo "  ✓ $RUST_CRATE_DIR/Cargo.lock (updated to latest compatible)"
    echo ""
  fi
  ...
  if [[ -n "$NODE_DIR" ]] && [[ -f "$(node_manifest)" ]]; then
    ...
    echo "  ✓ $(node_manifest)"
    [[ -f "$(node_lockfile)" ]] && echo "  ✓ $(node_lockfile)"
    echo ""
  fi
...
verify_components() {
  ...
  # Check pyproject.toml files (auto-detected; root + every detected service)
  print_info "Checking Python configuration files..."
  local pyproject_files=()
  [[ -f pyproject.toml ]] && pyproject_files+=("pyproject.toml")
  for service in "${PYTHON_SERVICES[@]}"; do
    pyproject_files+=("$service/pyproject.toml")
  done
  ...
  # Check Dockerfiles (auto-detected)
  print_info "Checking Dockerfiles..."
  local dockerfile_list=()
  while IFS= read -r -d '' f; do
    dockerfile_list+=("$(echo "$f" | sed 's|^\./||')")
  done < <(find . -maxdepth 3 -name Dockerfile \
    -not -path './.venv/*' -not -path './node_modules/*' \
    -not -path './target/*' -not -path './.worktrees/*' \
    -not -path './backups/*' -print0 2>/dev/null)
  ...
  if [[ -n "$RUST_CRATE_DIR" ]] && [[ -f "$(rust_manifest)" ]]; then
    print_info "Found Rust crate at $RUST_CRATE_DIR"
    total_components=$((total_components + 1))
    found_components=$((found_components + 1))
  fi
  ...
  if [[ -n "$NODE_DIR" ]] && [[ -f "$(node_manifest)" ]]; then
    print_info "Found Node project at $NODE_DIR"
    total_components=$((total_components + 1))
    found_components=$((found_components + 1))
  fi
